...
-- The app calls this when a user clicks the "Finish Workout" button.
-- Updates the template and the counters which track how
-- often the user has performed an exercise.
-- The procedure trusts the array order sent by the app when saving.
...
BEGIN
    ...
    -- Roll back the whole workout save if any statement fails.
    ...
    -- and the deletion of old rows requires the reading of the table contents to 
	...
	-- First delete the unneeded joins
	-- Delete unneeded sets joins (if user removed some sets)
    -- (Cross join was explicitly chosen because of how the data is structured)
	...
	-- Update and insert from here on out
...
-- Saves template changes without incrementing exercise completion counters.
...
BEGIN
    ...
    -- Putting everything into a transaction in case something breaks so it can be rolled back.
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN
		ROLLBACK;
        SIGNAL SQLSTATE '45000'
