...
def import_csv_to_table(cnx, cursor, data_dir: Path, csv_name: str, table_name: str, auto_increment_col: str | None):
    ...
    with filepath.open(mode="r", encoding="utf-8", newline="") as csv_file:
        ...
        query_columns = ", ".join([f"`{col}`" for col in headers])
        value_placeholders = ", ".join(["%s"] * len(headers))
        ...
        for line_number, row in enumerate(csv_reader, start=2):
            ...
            values = tuple(None if row_data[col].upper() == "NULL" else row_data[col] for col in headers)
...
def main():
    ...
    try:
        ...
        if args.truncate:
            ...
            for _, table_name, auto_increment_col in TABLE_IMPORT_CONFIG:
